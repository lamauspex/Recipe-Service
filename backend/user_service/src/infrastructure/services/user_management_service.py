...
class UserManagementService(UserManagementInterface):
    ...
    async def get_users(self, request: UserListRequestDTO) -> UserListResponseDTO:
        ...
        try:
            logger.info(
                f"Getting users list: page={request.page}, per_page={request.per_page}")
    ...
    async def update_user_status(self, request: UserStatusUpdateRequestDTO) -> UserStatusResponseDTO:
        ...
        try:
            logger.info(
                f"Updating user status: {request.user_id} -> {request.status}")
            ...
            logger.error(
                f"Error updating user status for {request.user_id}: {str(e)}")
    ...
    async def get_user_activity(self, request: UserActivityRequestDTO) -> UserActivityResponseDTO:
        ...
        try:
            logger.info(
                f"Getting user activity: {request.user_id}, days={request.days}")
            ...
            logger.error(
                f"Error getting user activity for {request.user_id}: {str(e)}")
    ...
    async def search_users(self, request: UserSearchRequestDTO) -> UserSearchResponseDTO:
        ...
        try:
            logger.info(
                f"Searching users: '{request.search_term}', limit={request.limit}")
            ...
            logger.error(
                f"Error searching users with term '{request.search_term}': {str(e)}")
