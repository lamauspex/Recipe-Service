...
from backend.services.recipe_service.src.models import Recipe, Ingredient, RecipeStep, Rating
...
class RecipeService:
    ...
    def create_recipe(self, recipe_data: RecipeCreate, author_id: UUID) -> RecipeResponse:
        ...
        for ingredient_name in recipe_data.ingredients:
            ingredient_data = {
                ...
                'quantity': None  # Можно добавить логику для парсинга количества
            }
    ...
    def get_recipes_list(self, search_params: RecipeSearchParams) -> List[RecipeResponse]:
        """Получение списка рецептов с фильтрацией"""
        recipes = self.recipe_repo.get_recipes_list(
            skip=search_params.skip,
            limit=search_params.limit,
            search=search_params.search,
            min_cooking_time=None,  # Можно добавить в схему поиска
            max_cooking_time=search_params.max_cooking_time
        )
        return [self._recipe_to_response(recipe) for recipe in recipes]

    def update_recipe(self, recipe_id: UUID, update_data: RecipeUpdate, author_id: UUID) -> RecipeResponse:
        """Обновление рецепта"""
    ...
    def add_review(self, review_data: ReviewCreate, author_id: UUID) -> Dict[str, Any]:
    ...
    def _recipe_to_response(self, recipe: Recipe) -> RecipeResponse:
        ...
        return RecipeResponse(
            ...
            difficulty=recipe.difficulty if hasattr(recipe, 'difficulty') else 'средне',
